(function () {
  ...
  function initMapboxChart(canvasId, unitLabel, dayCaption) {
    ...
    var filterEl = document.querySelector('[data-chart-filter="' + canvasId + '"]');
    ...
    function buildView(range) {
      if (range === 'month' || range === 'year') {
        ...
        return {
          ...
          caption: range === 'month'
            ? (unitLabel + ' por mês — últimos ' + order.length + ' meses')
            : (unitLabel + ' por ano — últimos ' + order.length + ' anos'),
        };
      }
          ...
          tooltip: {
            ...
            callbacks: {
              ...
              label: function (item) {
                ...
                return sign + entry.delta.toLocaleString('pt-BR') + ' ' + unitLabel + ' (total: ' + entry.used.toLocaleString('pt-BR') + ')';
              },
            },
          },
        },
  ...
  initMapboxChart('mapbox-chart', 'loads', 'Loads entre leituras registradas');
  initMapboxChart('mapbox-search-chart', 'requisições', 'Requisições entre leituras registradas');
})();
